...
const { CloudFrontClient, GetDistributionCommand, UpdateDistributionCommand, CreateCloudFrontOriginAccessIdentityCommand } = require("@aws-sdk/client-cloudfront");
...
const createResource = async (event, context, params) => {
  ...
  for (let i = 0; i < dynamicOrigins.length; i++) {
    existingDistributionConfig.Origins.Items.push({
      Id: `ssr-dynamic-assets-${i + 1}`,
      DomainName: dynamicOrigins[i].url,
  ...
  for (let i = 0; i < staticOrigins.length; i++) {
    const createOAIParams = { // CreateCloudFrontOriginAccessIdentityRequest
      CloudFrontOriginAccessIdentityConfig: { // CloudFrontOriginAccessIdentityConfig
        CallerReference: existingDistributionConfig.CallerReference, // required
        Comment: `OAI for ssr-dynamic-assets-${i + 1}`, // required
      },
    };
    console.log(createOAIParams);
    const createOAICommand = new CreateCloudFrontOriginAccessIdentityCommand(createOAIParams);
    const oaiResult = await client.send(createOAICommand);
    console.log(oaiResult);

    existingDistributionConfig.Origins.Items.push({
      Id: `ssr-static-assets-${i + 1}`,
      DomainName: staticOrigins[i].domainName,
      S3OriginConfig: {
        OriginAccessIdentity: oaiResult.CloudFrontOriginAccessIdentity.Id
      },
    });
    //TODO
  ...
  const updateDistributionInput = {
    DistributionConfig: {
      CallerReference: existingDistributionConfig.CallerReference,
      Origins: existingDistributionConfig.Origins
    },
    Id: process.env.DISTRIBUTION_ID,
  }
  console.log(updateDistributionInput);
