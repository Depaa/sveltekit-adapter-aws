...
const { CloudFrontClient, GetDistributionCommand, UpdateDistributionCommand, CreateOriginAccessControlCommand } = require("@aws-sdk/client-cloudfront");
...
const createResource = async (event, context, params) => {
  ...
  for (let i = 0; i < staticOrigins.length; i++) {
    const createOACParams = { // CreateCloudFrontOriginAccessIdentityRequest
      OriginAccessControlConfig: { // CloudFrontOriginAccessIdentityConfig
        Name: `ssr-dynamic-assets-${i + 1}`, // required
        Description: `OAC for ssr-dynamic-assets-${i + 1}`,
    ...
    const OACResult = await client.send(createOACCommand);
