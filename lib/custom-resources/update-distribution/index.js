  ...
  for (let i = 0; i < staticOrigins.length; i++) {
    const createOACParams = { // CreateCloudFrontOriginAccessIdentityRequest
      OriginAccessControlConfig: { // CloudFrontOriginAccessIdentityConfig
        Name: `ssr-static-assets-${i + 1}-${process.env.DISTRIBUTION_ID}`, // required
        Description: `OAC for ssr-static-assets-${i + 1}`,
    ...
    const bucketPolicy = await s3Client.send(getBucketPolicyCommand);
